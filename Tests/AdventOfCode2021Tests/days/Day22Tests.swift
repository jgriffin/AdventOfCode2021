...
final class Day22Tests: XCTestCase {
    ...
    func testAllCubesInput() {
        ...
        let onCubeCount = tree.onCount(inRange: nil)
        XCTAssertEqual(onCubeCount, 1_282_401_587_270_826)
    }
    ...
    func testAllCubesExample() {
        ...
        let onCubeCount = tree.onCount(inRange: nil)
        XCTAssertEqual(onCubeCount, 2_758_514_936_282_235)
    }
    ...
    func testFiftiesOnCubesInput() {
        ...
        let onCubeCount = tree.onCount(inRange: Self.fiftiesCuboid)
        XCTAssertEqual(onCubeCount, 610_196)
    }
    ...
    func testFiftiesOnCubesLargerExample() {
        ...
        let onCubeCount = tree.onCount(inRange: Self.fiftiesCuboid)
        XCTAssertEqual(onCubeCount, 590_784)
    }
...
extension Day22Tests {
    enum Step: Equatable {
        ...
        var cuboid: Cuboid {
            switch self {
            case let .on(c), let .off(c): return c
            }
        }

        func applyToTree(_ tree: inout CuboidTree) {
            switch self {
            case let .on(c): tree.setCuboid(c, on: true)
            case let .off(c): tree.setCuboid(c, on: false)
            }
        }
    }
